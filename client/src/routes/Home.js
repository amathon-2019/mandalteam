...
import { recentlyChartsQuery } from '../graphql/queries';
  ...
  render() {
    return (
      <div>
        ...
        <Query query={recentlyChartsQuery}>
          {({ loading, error, data }) => {
            if (loading) return <div style={ { textAlign: 'center'} }>잠시만 기다려주세요...</div>;
            if (error) return <div style={ { textAlign: 'center'} }>불러오기에 실패했습니다.</div>;
            const edges = data.allCharts.edges;

            return (
              edges.map((data) => <Room href={'/room/' + data.node.id} title={data.node.name}></Room>)
            )
          }}
        </Query>
      </div>
